...
/**
 * [removeDuplicates1 - Remove duplicates from the list using set container]
 * @param head [head of list]
 */
void removeDuplicates3( Node * head ) {
	...
	while( curr != nullptr ) {
    auto result = set.insert(curr->data);   
		if (result.second){
			prev->next = curr;
			curr = curr->next;
		}
...
int main() {
	...
	Node * head3 = nullptr;
	for ( int i = 0; i < 10; ++i ) {
		insert(head3, random_range(1,7));
	}
	printList(head3);
	removeDuplicates3(head3);
	printList(head3);
	return 0;
}
